...
const CardSelection = () => {
  ...
  const [currentPhase, setCurrentPhase] = useState('past') // past, present, future
  const [hoveredCard, setHoveredCard] = useState(null)
  const navigate = useNavigate()

  const phases = {
    past: { title: 'PASSÉ', subtitle: 'Ce qui vous a mené ici', index: 0 },
    present: { title: 'PRÉSENT', subtitle: 'Votre situation actuelle', index: 1 },
    future: { title: 'FUTUR', subtitle: 'Ce qui vous attend', index: 2 }
  }

  const handleCardSelect = (card) => {
    const newSelectedCards = [...selectedCards, { ...card, phase: currentPhase }]
    ...
    if (currentPhase === 'past') {
      ...
      // Toutes les cartes sélectionnées, naviguer vers la prédiction
      setTimeout(() => {
        navigate('/prediction', { state: { selectedCards: newSelectedCards } })
      }, 1500)
    }
  }

  const getCurrentPhaseText = () => {
    if (selectedCards.length === 3) return 'DESTINÉE'
    return phases[currentPhase].title
  }
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div 
        ...
        style={{
          backgroundImage: `url('/WhatsApp Image 2025-06-25 à 14.39.02_703a96b5.jpg')`,
      ...
      <div className="relative z-10 min-h-screen pt-20 px-8">
        ...
        >
          ...
          <span className="font-elegant text-sm tracking-wider">RETOUR AU MENU PRINCIPAL</span>
        </motion.button>
            ...
            {Object.entries(phases).map(([key, phase]) => (
              <motion.div
                ...
                animate={{
                  ...
                  opacity: selectedCards.find(card => card.phase === key) ? 1 : currentPhase === key ? 0.8 : 0.4
                }}
          ...
          {tarotCards.map((card, index) => (
            ...
            >
              <TarotCard
                ...
                disabled={selectedCards.some(selected => selected.id === card.id)}
              ...
              {/* Hover Info */}
              <AnimatePresence>
                {hoveredCard === card.id && !selectedCards.some(selected => selected.id === card.id) && (
                  <motion.div
                  ...
                  >
                    <p className="text-mystique-gold text-xs font-elegant text-center whitespace-nowrap">
                      {card.name}
                    </p>
